...
func (server *Server) CreateFaturaOTB(w http.ResponseWriter, r *http.Request) {
	...
	if err := config.AuthMod(w, r, 16011); err != nil {
	...
	if err = json.Unmarshal(body, &faturaOTB); err != nil {
	...
	//	Retorna um erro caso nao seja possivel salvar faturaOTB no banco de dados, Status 500
	if err != nil {
...
func (server *Server) GetFaturaOTBByID(w http.ResponseWriter, r *http.Request) {
	...
	allFaturaOTB, err := faturaOTB.FindFaturaOTB(server.DB, codOtb, numNF, codIbge)
	if err != nil {
		formattedError := config.FormatError(err.Error())
