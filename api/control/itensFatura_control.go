...
func (server *Server) GetAllItensFatura(w http.ResponseWriter, r *http.Request) {
	...
	allItensFatura, err := itensFatura.FindAllItensFatura(server.DB, numNF)
	if err != nil {
		formattedError := config.FormatError(err.Error())
...
func (server *Server) UpdateItensFatura(w http.ResponseWriter, r *http.Request) {
	...
	updateItensFatura, err := itensFatura.UpdateItensFatura(server.DB)
	if err != nil {
		formattedError := config.FormatError(err.Error())
...
func (server *Server) DeleteItensFatura(w http.ResponseWriter, r *http.Request) {
	...
	_, err = itensFatura.DeleteItensFatura(server.DB, numNF, idEmpenho, codItem, codTipoItem)
	if err != nil {
		formattedError := config.FormatError(err.Error())
