...
/*  =========================
	FUNCAO PARA ADICIONAR USUARIO MODULO
=========================  */
...
func (server *Server) AddModulo(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		...
		for _, data := range usuarioModulo {
			...
			err = logUsuarioModulo.LogUsuarioModulo(server.DB, data.CodUsuario, data.CodModulo, "usuario_modulo", "i", tokenID)
			if err != nil {
			...
			_, err := data.SaveModulo(server.DB)
			if err != nil {
				formattedError := config.FormatError(err.Error())
