...
func (log *Log) LogCDItens(db *gorm.DB, codIbge uint32, codItem uint32, codTipoItem uint32, nomeTabela string, operacao string, codUsuario uint32) error {
	...
	db.Table("cd_itens").Select("CONCAT(IFNULL(cod_ibge, ''), ';', IFNULL(cod_lote, ''), ';', IFNULL(cod_item, ''), ';', IFNULL(cod_tipo_item, ''), ';', IFNULL(quantidade_previsto, ''), ';', IFNULL(quantidade_projeto_executivo, ''), ';', IFNULL(quantidade_termo_instalacao, ''))").Where("cod_ibge = ?, AND cod_item = ? AND cod_tipo_item = ?", codIbge, codItem, codTipoItem).Row().Scan(&log.Espelho)
...
func (log *Log) LogEmpenho(db *gorm.DB, idEmpenho uint32, nomeTabela string, operacao string, codUsuario uint32) error {
	...
	err := db.Debug().Exec("INSERT INT log(cod_usuario, nome_tabela, operacao, espelho, cod_int_1 VALUES (?, ?, ?, ?, ?))", codUsuario, nomeTabela, operacao, log.Espelho, idEmpenho).Error
